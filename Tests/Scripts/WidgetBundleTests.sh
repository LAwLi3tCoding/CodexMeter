...
[[ "$(plist_value "$WIDGET_INFO" CFBundleShortVersionString)" == "0.3.2" ]] \
  || fail "widget short version is not 0.3.2"
[[ "$(plist_value "$WIDGET_INFO" CFBundleVersion)" == "5" ]] \
  || fail "widget build version is not 5"
