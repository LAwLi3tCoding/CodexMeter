...
[[ "$(plist_value "$WIDGET_INFO" CFBundleShortVersionString)" == "0.4.0" ]] \
  || fail "widget short version is not 0.4.0"
[[ "$(plist_value "$WIDGET_INFO" CFBundleVersion)" == "6" ]] \
  || fail "widget build version is not 6"
