...
[[ "$(plist_value "$WIDGET_INFO" CFBundleShortVersionString)" == "0.4.3" ]] \
  || fail "widget short version is not 0.4.3"
[[ "$(plist_value "$WIDGET_INFO" CFBundleVersion)" == "9" ]] \
  || fail "widget build version is not 9"
