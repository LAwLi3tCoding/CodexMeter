...
[[ "$(plist_value "$WIDGET_INFO" CFBundleShortVersionString)" == "0.4.1" ]] \
  || fail "widget short version is not 0.4.1"
[[ "$(plist_value "$WIDGET_INFO" CFBundleVersion)" == "7" ]] \
  || fail "widget build version is not 7"
