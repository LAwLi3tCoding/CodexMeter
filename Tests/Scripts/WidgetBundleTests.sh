...
[[ "$(plist_value "$WIDGET_INFO" CFBundleShortVersionString)" == "0.4.2" ]] \
  || fail "widget short version is not 0.4.2"
[[ "$(plist_value "$WIDGET_INFO" CFBundleVersion)" == "8" ]] \
  || fail "widget build version is not 8"
